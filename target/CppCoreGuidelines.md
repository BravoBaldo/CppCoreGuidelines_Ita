...
* [Baldassarre Cesarano](baldassarre.cesarano (AT) libero.it)
...
* I gruppi di regole non sono stati completamente controllati per quanto riguarda la completezza, la consistenza e l'obbligatorietà.
...
[=, ctors e dtors [costruttori e distruttori]](#S-ctor) --
[gerarchie](#SS-hier) --
...
[potrebbe non fallire](#Rc-dtor-fail)
* eccezione:
[errori](#S-errors) --
[`throw`](#Re-throw) --
...
Le estensioni spesso non hanno una semantica rigorosamente definita.  Anche le estensioni comuni e implementate da più compilatori possono avere comportamenti leggermente diversi e il comportamento del caso limite come diretta conseguenza del *non* avere una definizione standard rigorosa.  Con un sufficiente uso di una di queste estensioni, ci si dovrà aspettare un impatto sulla portabilità.
...
Lo scopo del "solo" ciclare sugli elementi di `v` qui non è espresso. I dettagli di implementazione di un indice sono esposti (quindi se ne può abusare), e `i` sopravvive allo scope del ciclo, che può o meno essere voluto. Il lettore non può capire solo da questo pezzo di codice.
...
##### Esempio, cattivo
    ...
    // compilato separatamente, possibilmente caricato dinamicamente
    extern void f(int* p);
...
Si deve passare il puntatore e il numero di elementi come un unico oggetto:
    ...
    extern void f4(vector<int>&);   // compilato separatamente, possibilmente caricato dinamicamente
...
Anche una crescita lenta delle risorse, col tempo, ne esaurirà la disponibilità.
...
Combinarlo con l'applicazione dei [profili di tipo e bound](#SS-force) ottenendo una completa sicurezza di tipo e risorse, garantita dai tool.
...
Sì, questa è una caricatura, ma abbiamo visto ogni singolo errore nel codice di produzione, e anche peggio.
Si noti che il layout di `X` garantisce la perdita di almeno 6 byte (e molto probabilmente di più).
La definizione spuria delle operazioni di copia disabilita la semantica del move rallentando l'operazione di ritorno (si noti che il Return Value Optimization [Ottimizzazione del Valore di Ritorno], RVO, quì non è garantita).
...
Ad esempio, invece di fare confusione con le matrici, le union, i cast, problemi coi cicli di vita, `gsl::owner`, ecc.,
...
I tipi `std::chrono::duration` (C++11) aiutano a rendere esplicita l'unità di durata del tempo.

    void blink_led(milliseconds time_to_blink) // buono -- l'unità è esplicita
    {
    // ...
    // fa qualcosa con time_to_blink
    // ...
...
Gli argomenti hanno un significato che può limitarne l'uso corretto nel chiamato.
...
In tal caso, contrassegnare i puntatori proprietari utilizzando `owner` dalla [Guidelines Support Library](#S-gsl):
    ...
    owner<X*> compute(args)    // Ora è chiaro il trasferimento dell'ownership
    {
    owner<X*> res = new X{};
    // ...
    return res;
...
##### Esempio
    ...
    int length(const char* p);            // non è chiaro se la lunghezza(nullptr) sia valida
...
L'assunzione che il puntatore a `char` punti ad una stringa C-style (una stringa di caratteri terminata con zero) era ancora implicita ed una potenziale sorgente di confusione e di errori. Usare `czstring` preferendola a `const char*`.
    ...
    // possiamo supporre che p non possa essere nullptr
...
Cosa succede se sono presenti meno di `n` elementi nell'array puntato da `q` ? Allora, probabilmente si sovrascrive della memoria estranea [unrelated].
Cosa succede se nell'array sono presenti meno di `n` elementi nell'array puntato da `p`? Allora, probabilmente, si leggerà da una memoria estranea [unrelated].
...
**Alternative**: Utilizzare una classe di supporto per garantire che il numero di elementi sia corretto e per prevenire conversioni implicite pericolose. Per esempio:
...
Poiché `x` e `y` stanno in unità di traduzione diverse, l'ordine delle chiamate a `f()` e `g()` non è definito; 
si accederà ad un `const` non inizializzato.
...
Manca un'astrazione, quindi è un valore composto passato come singoli elementi anziché come un singolo oggetto che impone [enforces] un invariante.
Questo non solo espande la lista dei parametri, ma porta ad errori perché i valori dei componenti non sono più protetti da un invariante imposto [enforced].
...
Si noti che ciò è dovuto al problema 1 precedente -- mancanza di un'astrazione. Anziché passare un range (astrazione), la STL passa una coppia di iteratori (calori dei componenti non incapsulati).

Qui, si hanno quattro argomenti template e sei argomenti funzione.
Per semplificarne gli usi più frequenti e più semplici, si può assegnare, come default, all'argomento di confronto `<`:
...
I 'Profile' di sicurezza consigliano di sostituire

    void f(int* some_ints, int some_ints_length);  // MALE: in stile C, non sicuro

con

    void f(gsl::span<int> some_ints);              // BUONO: sicuro, controllo dei limiti
...
**Alternative**: Usare un'astrazione migliore: Raggruppare gli argomenti in oggetti significativi e passare gli oggetti (per valore o per riferimento).

**Alternative**: Usare gli argomenti di default o gli overload per consentire le forme più comuni di chiamate da effettuarsi con pochi argomenti.

##### Imposizione

* Avvisare quando una funzione dichiara due iteratori (compresi i puntatori) dello stesso tipo anzziché un range o una view.
* (Non imponibile) Questa è una linea-guida filosofica che è impossibile da controllare direttamente.
...
Si sapeva che `Shape` sarebbe comparso da qualche parte :-)
    ...
    class Shape {  // male: classe interfaccia caricata con dati
    public:
...
Su alcune piattaforme stanno emergendo delle ABI comuni (Application Binary Interface) [Interfaccie Binarie di Applicazioni] che liberano dalle restrizioni più draconiane (rigorose).
...
Cfr. [GOTW #100](https://herbsutter.com/gotw/_100/) e [cppreference](http://en.cppreference.com/w/cpp/language/pimpl) per valutazioni [trade-offs] e ulteriori dettagli implementativiassociati a questo idioma.
...
In tal caso, si mantengono locali, anziché "infettare" le interfacce in modo che gruppi più grandi di programmatori siano consapevoli delle sottigliezze.
...
Ciò viola la regola [contro le variabili non inizializzate](#Res-always),
la regola contro [il trascurare l'ownership](#Ri-raw),
e la regola [contro le costanti magiche](#Res-magic).
...
* Difficile, è difficile decidere quale codice rompi-regola è essenziale
...
* [F.4: Se una funzione può essere valutata in fase di compilazione, la si dichiara `constexpr`](#Rf-constexpr)
* [F.5: Se una funzione è molto piccola e time-critical, la si dichiara inline](#Rf-inline)
* [F.6: Se la funzione non può andare in errore [throw], la si dichiara `noexcept`](#Rf-noexcept)
...
* [F.18: I parametri "si-sposterà", si passano per `X&&` e `std::move`](#Rf-consume)
* [F.19: I parametri "forward", si passano per `TP&&` e solo `std::forward` il parametro](#Rf-forward)
* [F.20: Per i valori "out", preferire i valori di ritorno ai parametri di output](#Rf-out)
* [F.21: Per il return di valori "out" multipli, preferire il ritorno di una struct o una tupla](#Rf-out-multi)
...
* [F.53: Evitare l'acquisizione per riferimento nelle lambdas che si non si useranno localmente, compresi quelli restituiti, memorizzati nell'heap o passati da un altro thread](#Rf-value-capture)
...
Se fosse necessario, si potrebbero ulteriormente 'templatizzare' `read()` e `print()` sul tipo di dato, il maccanismo di I/O, la risposta agli errori, ecc. Esempio:
...
### <a name="Rf-constexpr"></a>F.4: Se una funzione può essere valutata in fase di compilazione, la si dichiara `constexpr`
...
Qualsiasi API che possa eventualmente dipendere da una configurazione a run-time di alto livello o da logiche aziendali non dovrebbe essere `constexpr`. Tale personalizzazione non può essere valutata dal compilatore, e qualsiasi funzione `constexpr`che dipenda da quell'API dovrebbe essere riscritta o non dev'essere `constexpr`.
...
### <a name="Rf-noexcept"></a>F.6: Se la funzione non può andare in errore [throw], la si dichiara `noexcept`
...
Le funzioni `constexpr`possono andare in errore quando vengono valutate a run time, quindi potrebbe essere necessario definirne alcune `noexcept`.
...
A meno che il programma non sia concepito per sopravvivere all'esaurimento della memoria, questa potrebbe essere la cosa giusta da fare; `terminate()` può generare informazioni adeguate nel log degli errori (ma dopo aver esaurito la memoria è difficile fare qualcosa di intelligente).

##### Note

Si deve essere consapevoli dell'ambiente di esecuzione in cui gira il proprio codice quando si decide di dichiarare una funzione `noexcept`, specialmente per i problemi causati dagli errori e dalle allocazioni.  Un codice che si intende essere perfettamente generale (come la libreria standard ed il codice di altre utility simili) deve supportare ambienti dove una ecezione `bad_alloc` può essere gestita in modo significativo.
Tuttavia, la maggior parte dei programmi e degli ambienti di esecuzione non è in grado di gestire in modo significativo un errore di allocazione e, in quei casi, l'interruzione del programma è la risposta più pulita e semplice a un errore di allocazione.  Se si sa che il proprio codice non possa risponderead un errore di allocazione, può essere opportuno aggiungere `noexcept` anche alle funzioni che allocano.
...
##### Esempio
    ...
    X* find(map<Blob>& m, const string& s, Hint);   // c'era una volta un hint
...
Quando la copia è economica, nulla ne batte la semplicità e la sicurezza e per gli oggetti piccoli (fino a due o tre word) è anche più veloce del passaggio per riferimento perché non richiede, alla funzione, un'indirezione extra per l'accesso.
...
##### Esempio
    ...
    int multiply(int, int); // soli int in input, si passano per valore
...
Supponendo che `Matrix` abbia le operazioni move (possibilmente mantenendo i suoi elementi in un `std::vector`):
...
### <a name="Rf-consume"></a>F.18:  I parametri "si-sporterà", si passano per `X&&` e `std::move` the parameter
...
L'argomento per aggiungere `const` ad un valore di ritornoè che impedisce (rarissimamente) accessi accidentali a un elemento temporaneo.
...
* Se un tipo è cstoso da subire un move (p.es., `array<BigPOD>`), si consideri di allocarlo nel [free store] e restituirne un handle (p.es., `unique_ptr`), o passandolo in un riferimento ad un oggetto destinazione non-`const` da riempire (da usarsi come parametro di uscita).
...
Col C++11 lo possiamo scrivere mettendo il risultato direttamente nelle variabili locali esistenti:
    ...
    UnCertoTipo iter;                                // il default inizializza se non abbiamo già
...
In molti casi, può essere utile restituire un tipo specifico definito dall'utente.
Per esempio:
...
Chiarezza. Una funzione con un parametro `not_null<T>`  chiarisce che il chiamante della funzione è responsabile di tutti i controlli `nullptr` necessari.
...
Si presume che un `not_null<T*>` non sia `nullptr`; un `T*` può essere `nullptr`; entrambi si possono rappresentare in memoria come un `T*` (quindi non è implicito alcun overhead a run-time).
...
### <a name="Rf-assignment-op"></a>F.47: Restituire `T&` dagli operatori di assegnament
...
oppure
    ...
    void print(const string& s);  // usa il formato col default
    void print(const string& s, format f);
...
Un set di overload può avere dei membri che non accedono direttamente ai dati `private`:
...
##### Esempio
    ...
    namespace Chrono { // qui teniamo i servizi relativi al tempo
...
I dati privatisono nascosti lontano da quelli pubblici.
...
I tipi predefiniti del C++ sono regulari, così come le classi della standard-library, come `string`, `vector` e `map`.
...
* [C.84: Una funzione `swap` non può fallire](#Rc-swap-fail)
...
Dato che era necessaria un'"attenzione speciale" per il distruttore (qui, per deallocare), la probabilità che l'assegnazione di copia e di [move] (entrambi distruggono implicitamente un object) siano corrette, è bassa (qui, ci sarebbe una doppia cancellazione).

##### Note

Questa è notacome "la regoa del cinque" o "la regola del sei", a seconda che si conti o meno il costruttore di default.
...
Quando un distruttore dev'essere dichiarato solo per renderlo `virtual`, può essere definito come di default. Per evitare la soppressione di operazioni [move] implicite, queste devono essere anche dichiarate, poii, per evitare che la classe diventi [move-only] (e non copiabile), si devono dichiarare le operazioni di copia:
...
Per le risorse rappresentate come classi con un complet set di operazioni di default, ciò avviene automaticamente.
...
* (Difficile) Determinare se le variabili membro puntatori o riferimentisono proprietari quando non c'è alcuna istruzione esplicita sulla proprietà (p.es., guardando nei costruttori).
...
Osservare l'inizializzazione dei puntatori menbro e dei riferimento ai membri e vedere se viene utilizzata un'allocazione.
...
Un membro puntatore può rappresentare una risorsa.
...
Se una risorsa non può essere rilasciata e il programma non può fallire, si provi a segnalare l'errore al resto del sistema in qualche modo (forse anche modificando qualche stato globale e sperando che qualcosa si accorga e sia in grado di provvedere).
...
Se un distruttore esegue operazioni che potrebbero non riuscire, può rilevare delle eccezioni e in qualche caso completare comunque correttamente (p.es., utilizzando un meccanismo diverso da quello che è andato in errore).
...
[Un distruttore non può fallire](#Rc-dtor-fail). Se un distruttore tenta di uscire con un'eccezione, si tratta di un errore di progettazione e sarebbe meglio che il programma terminasse.
...
##### Esempio
    ...
    vector<Date> vd2(1000, Date{Month::October, 7, 1885});   // alternativamente
...
Questo va bene ed è generale, ma impostando un `Vector0` a vuoto dopo un errore implica un'allocazione, che potrebbe non riuscire.
Inoltre, avere un `Vector` di default rapresentato come `{new T[0], 0, 0}` sembra dispendioso.
...
**Alternativa**: Possiamo parzialmente avvantaggiarci dei benefici degi argomenti di default nei costruttori, e che non è raro nel codice più vecchio. Tuttavia, questo è meno esplicito, causa l'esistenza di più argomenti da passare, ed è ripetitivo quando ci sono più di un costruttore:
...
L'azione comune diventa noiosa da scrivere e può accidentalmente non essere comune.
...
Ecco un modo per spostare un puntatore senza un test (lo si immagini come codice nell'implementazione di un assegnazione con spostamento):
    ...
    // sposta da un other.ptr a this->ptr
    T* temp = other.ptr;
    other.ptr = nullptr;
    delete ptr;
    ptr = temp;
...
* Segnalauna classe polimorfica con un'operazione di copia non-cancellata.
...
Questo non è soltanto lento, ma se per gli elementi avviene un'allocazione di memoria in `tmp`, questo `swap` può fallire e farebbe fallire gli algoritmi STL se venissero usati.
...
[Uno `swap` non può fallire](#Rc-swap-fail).
...
Il meccanismo standard del C++ per costruire un'istanza di un tipo consiste nel richiamare il suo costruttore. Come specificato nella linea-guida [C.41](#Rc-complete): uncostruttore dovrebbe creare un oggetto completamente inizializzato. Non dovrebbe essere necessaria alcuna ulteriore inizializzazione, come per `memcpy`.
...
* `virtual` ignifica esattamente e solo "questa è una nuova funzione virtuale".
...
* l'ereditarietà dell'interfaccia è l'utilizzo dell'ereditarietà per separare gli utenti dalle implementazioni, in particolare consente dialle classi derivate di poter essere aggiunte e modificate senza influire sugli utenti delle classi base.
...
##### Esempio
    ...
    use(a);       // bad: a decade in &a[0] che è convertito in un B*
...
Una chiamata a una funzione virtuale raggiunge le funzioni più derivate, mentre un cast può raggiungere una classe intermedia dando, quindi, un risultato sbagliato (specialmente durante la modifica per manutenzione di una gerarchia).
...
Si consideri:
    ...
    void open_gate(Gate& g);   // apre la porta del garage
    void fopen(const char* name, const char* mode);   // apre un file

Le due operazioni sono fondamentalmente diverse (e non correlate), quindi è bene che i loro nomi differiscano. Al contrario:

    void open(Gate& g);   // apre la porta del garage
    void open(const char* name, const char* mode ="r");   // apre un file
...
##### Esempio
    ...
    // Ottimizzazione di stringhe corte
...
* Segnalare i vluri duplicati degli enumeratori
...
In questo codice ci si deve ricordare di `unlock`, `close_port`, e `delete` in ogni diramazione del fluss di codice, e farlo esattamente una sola volta.
...
### <a name="Rr-global"></a>R.6: vitare le variabili globali non-`const`
...
In caso contrario, un'eccezione o un return possono causare un leak.
...
Questo non è exception-safe perché il compilatore può riordinare le due espressioni  nel costruire i due argomenti della funzione.
...
Un array si riduce ad un puntatore, perdend così la propria dimensione, aprendo la possibilità di errori sul range.
...
Si consideri:
    ...
    shared_ptr<X> p1 { new X{2} }; // male
    auto p = make_shared<X>(2);    // bene
...
##### Esempio
    ...
    void reseat(unique_ptr<widget>&); // "potrà" o "potrebbe" eseguire il reseat di un puntatore
...
Per le regole sula nomenclatura, sui commenti e l'indentazione, cfr.[NL: Nomenclatura e layout](#S-naming).
...
##### Esempio
    ...
    auto sum = accumulate(begin(a), end(a), 0.0);   // buono
...
Una dichiarazione è un'istruzione. Una dichiarazione introduce un nome in uno scope e può causare la costruzione di un oggetto.
...
* Segnala quando le risorse costose, come gli handle dei fie e i lock nn vengono usati per N-righe (con un N adatto
...
##### Esempio
    ...
    auto lst = { 1, 2, 3 };   // lst è un elenco di inizializzatori
    auto x{1};   // x è un int (in C++17; initializer_list in C++11)
...
##### Esempio, cattivo
    ...
    SomeLargeType var;   // un brutto CaMeLcAsEvArIaBlE
...
* Segnalare gli array con dimensioni [bound] non-costanti (VLA [Variable Lenght Array] del C)
...
##### Esempio, cattivo
    ...
    widget x;   // dovrebbe essere const, ma:
...
Come è solito per le macro, ci sono usi "per lo più innocui", ma anche questi possono creare problemi ai tool, come i completatori automatici, gli analizzatori statici, e i debugger.
...
##### Esempio, buono
    ...
    void f(span<int> a) // BETTER: usa span nella dichiarazione della funzione
    {
    if (a.size() < 2) return;
...
Se si vuole passare un array, diciamo:
    ...
    void g(int* p, size_t length);  // vecchio codice (pericoloso)
...
Tuttavia, si ricordi che il proprio codice potrebbe essere compilato con un compilatore pre-C++17 (p.es., tramite il taglia-incolla) quindi non comportarsi troppo da esperti.
...
La libreria di supporto delle linee guida offre un'operazione `narrow_cast` per specificare che il narrowing è accettabile e un `narrow` ("narrow if") che genera un'eccezione se un troncamento butta via delle informazioni:
    ...
    i = narrow_cast<int>(d);   // OK (lo si è chiesto): narrowing: i becomes 7
...
* Segnalare tutti i `long`->`char` (Si sopetta che `int`->`char` sia molto comune. Questo è un punto oscuro! c'è bisogno di dati).
...
Segnalare l'uso di `0` e `NULL` per i puntatori. La trasformazione può essere aiutata da una semplice trasformazione del programma.
...
##### Esempio, cattivo
    ...
    f(j); // comportamento incerto

##### Esempio

Talvolta, si può essere tentati di ricorrere a `const_cast` per evitare la duplicazione del codice, ad esempio quando si hanno due funzioni di accesso che differiscono solo per un `const` ma con implementazioni simili. Per esempio:
...
##### Esempio
    ...
    for (auto& x : v)      // scrive tutti gli elementi di v
    cout << x << '\n';
...
##### Esempio, cattivo
    ...
    vector<int> v = std::move(make_vector());   // non buono; std::move è completamente ridondante
...
* Segnalare quando `std::forward` viene applicato ad un riferimenro diverso dal forwarding. (Un caso più generale della regola precedente per coprire i casi non-moving).
...
Si può definire un tipo per rappresentare il numero degli elementi:
    ...
    Vector<Count> v3{Count{10}};    // sì, c'è ancora un problema molto piccolo
...
uesta è una parte importante della discussione sul [modello del C++ per la sicurezza dei tipi e delle risorse](#Stroustrup15).
...
Per risolvere il problema, o si prolunga la durata dell'oggetto a cui si intende fare riferimento col puntatore, o ridurre òa durata del puntatore (spostare la dereferenza prima che finisca la durata dell'oggetto puntato).
...
Esistono molti approcci per affrontare questo potenziale problema:
    ...
    void f1(int* p) // avere a che fare con nullptr
    {
    if (!p) {
...
* Segnalare una dereferenza di un puntatore che può essere stato invalidato da un `delete`
* Segnalare una dereferenza di un puntatore a un elemento di un contenitore che può essere stato invalidato per dereferenza
...
##### Esempio
    ...
    for (gsl::index i = 0; i < v.size(); ++i)   // non buono
    cout << v[i] << '\n';
...
Un umano o un buon analizzatore statico può determinare che non ci sono effetti collaterali su `v` in `f(v, &v[i])` in modo che si possa riscrivere il loop.
...
##### Esempio
    ...
    for (int i = 0; i < 100; ++i) {   // BENE: i è visibile solo all'interno del loop
    // ...
    }

##### Esempio, da non fare

    int j;                            // BAD: j è visibile all'esterno del loop
    for (j = 0; j < 100; ++j) {
    // ...
    }
...
Cè un largo uso del blocco goto-exit del C:
...
* Segnalare i `goto`. Maglio ancora segnalare tutti i `goto` che non saltano da un ciclo nidificato all'istruzione immediatamente al ciclo più esterno.
...
La dimenticanza di un case solitamente avviene quando viene aggiunto un elemento ad un enumerazione e chi lo ha fatto non è riuscito a inserilo in tutti gli switch che usano quell'enumeratore.
...
##### Esempio
    ...
    for (i = 0; i < max; ++i);   // NON BUONO: l'istruzione vuota viene facilmente ignorata (non vista)
    v[i] = f(v[i]);
...
Questa regola è particolarmente utile quando una dichiarazione viene utilizzata come condizione
    ...
    if (auto pc = dynamic_cast<Circle>(ps)) { ... } // eseguito se ps punta ad un tipo di Circle, bene
...
##### Esempio, cattivo
    ...
    auto a = area(10'000'000, 100'000'000);   // non buono
...
Eseguire un codice single-threaded esistente concorrentemente può risultare banale come aggiungere strategicamente `std::async` o `std::thread`, o può essere necessario riscrivere tutto, a seconda che il codice originale sia stato scritto in modo thread-friendly.
...
I dati immutabii si possono condividere i modo sicuro ed efficientemente.
...
Se non si sa cosa fa `Foo::act` (forse è una funzione virtuale che invoca una classe membro derivata da una classe non ancora scritta), può chiamare `do_this` (ricorsivamente) provocando un deadlock su `my_mutex`.
...
Un esempio comune del problema di "chiamare codice ignoto" è una chiamata a una funzione che tenta di acquisire l'accesso allo stesso oggetto lockato.
...
Con "bad" si intende che un `thread` può usare un puntatore dopo che l'oggetto puntato è stato distrutto.
...
Segnalare i tentativi di passare variabii locali ad un thread di cui si può eseguire il `detach()`.
...
Gli usi del pattern "double-checked locking" che non sono in volazione della [CP.110: Non scriversi il proprio "interblocco ricontrollato" [double-checked locking] per l'inizializzazione](#Rconc-double), si hanno quando un'azione "non-thread-safe" è sia difficile [hard] che rara, ed esiste un test veloce "thread-safe" utilizzabile per garantire che l'azione non è necessaria, ma non è utilizzabile per garantire il contrario.
...
Un preciso ordine della memoria può essere d'aiuto dove il carico di acquisizione è più efficiente di quello sequenziale
...
Per esempio, leggendo `clock` die volte si hanno spesso due valori diversi, quindi è opportuno che l'ottimizzatore non ottimizzi la seconda lettura in questo codice:
...
A volte il codice C++ code alloca memoria `volatile` e la condivide con "qualcun altro" (hardware o un altro linguaggio) per sfuggire deliberatamente ad un puntatore [escaping a pointer]:
    ...
    please_use_this(&vl);   // strappare un riferimento a questo "altrove [escape a reference to this to "elsewhere"] (non ilC++)
...
##### Esempio
    ...
    void f1(int i)   // Non buono: possibile leak
    {
    int* p = new int[12];
    // ...
...
Molte funzioni della libreria standard sono `noexcept` comprese tute quelle "ereditate" dalla Libreria Standard del C.
...
##### Esempio
    ...
    void leak(int x)   // da non fare: può essere un leak
    {
    auto p = new int{7};
...
Per esempio, si potrebbe mettere un socket che non vuoe chiudersi in una lista dei "socket cattivi", per poi esaminare la lista con una regolare scansione dello stato del sistema.
...
### <a name="Re-finally"></a>E.19: sare un oggetto `final_action` per la pulizia [cleanup] se non è disponibile un gestore delle risorse adatto
...
Il problema è che `f()` può stare in una libreria di cui non si ha il controllo e la nuova eccezione è qualcosa con cui `use()` non può farci nulla o a cui non è in qualche modo interessata.
...
Esempio:
    ...
    void f(int* p);   // vecchio codice: f() non modifica `*p`
...
* Non segnalare la definizione degli stessi concetti "eccessivamente semplici"; questi possono semplicemente essere elementi costitutivi di concetti più utili.
...
Questo può gonfiare la dimensione del codice, e può eccessivamente vincolare un tipo generico istanziando funzionalità che non saranno mai necessarie.
...
##### Esempio (utilizzando i concetti TS)
    ...
    auto& x = v.front();     // bad
    String& s = v.front();   // buono (String è un concetto GSL)
...
Un assioma può non essere generale, ma chi scrive il template può presumere che valga per tutti gli input effettivamente utilizzati (simile a una pre-condizione).
...
I [rait] possono essere classi [trait]  o [trait] di tipi.
...
Si noti che anche l'utilizzo di nomi non locali e non dipendenti (come `debug` e `cerr`) introduce dipendenze dal contesto che possono portare a "misteriosi" errori.
...
Per esempio, ul algoritmo solitamente usa altri algoritmi e invoca operazioni che non operano esclusivamente su argomenti.
...
##### Esempio
    ...
    ??? specializzazione di string (==)
    
    ??? specializzazione della rappresentazione ?
...
##### Esempio
    ...
    f(1, "asdf"); // non buono: il cast da const char* a int
...
### <a name="Rt-variadic-pass"></a>T.101: ??? Coe passare gli argomenti ad un template variadico ???
...
##### Esempio
    ...
    ??? attenzione agli argomenti [move-only] e ai riferimenti
...
Ci sono modi piò precisi per indicare una sequenza omogenea, come `initializer_list`.
...
##### Esempio
    ...
    ??? ottimizzazione oggetto grande / oggetto piccolo
...
* Segnalare `x.size() == 0` quando è disponibile `x.empty()` o `x.is_empty()`. Controllare se vuoto, funziona per più contenitori rispetto a size(), perché alcui contenitori non conoscono la propria dimensione [size] o sono concettualmente di dimensioni illimitate.
...
Da qualche parte, forse in un file di implementazione, lasciare che sia il compilatore a verificare le proprietà desiderate di `X`:
    ...
    static_assert(Default_constructible<X>);    // errore: X non ha un default constructor
...
##### Esempio
    ...
    int* p1 = malloc(10 * sizeof(int));                      // non C++
...
Ad esempio, per utilizzare una libreria C di terze parti o un'interfaccia di sistemi C, definire l'interfaccia a basso livello nel sottinsieme comune al C e al C++ per un miglior controllo dei tipi.
...
In questo documento, ci si riferisce a `.h` e `.cpp` come abbreviazioni per file header e file di implementazione, anche se l'effettiva estensione può essere diversa.
...
### <a name="Rs-inline"></a>SF.2: Un file `.h` potrebbe non contenere definizioni di oggetti o definizioni di funzioni non-inline
...
* un namespace (un "namespace di implementazione") pro proteggere da molte dipendenze dal contesto.
* la totale protezione e flessibilità si ottiene con i mooduli.

**Si veda anche**:

* [Bozza di Lavoro, Estensioni al C++ per i Moduli [Working Draft, Extensions to C++ for Modules]](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/n4592.pdf)
* [Moduli, Componentizzazione e Transizione [Modules, Componentization, and Transition]](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0141r0.pdf)
...
`using namespace` può portare a cnflitti con i nomi, quindi dovrebbe essere usato con parsimonia.
...
Assicurarsi di includere anche una chiave ed un buon differenziator, come il nome della libreria o del componente di cui fa parte il file header.
...
L'aggiunta a `std` può modificare il significato di codice conforme agli standard.
Le aggiunte a `std` possono creare conflitti con le future versioni dello standard.
...
* Segnalare la dichiarazione di un array C in una funzione o una classe che dichiara anche un contenitore STL (per evitare un un numero eccessivo di avvisi per il codice ereditato non-STL). Per rsolvere: Modificare almeno l'array the in uno `std::array`.
...
Non dare per scontato che `string` sia pià lenta delle tecniche  basso livello senza fare delle misure e si ricordi che non in tutto il codice sono importanti le prestazioni.
...
Se presenti nella propria libreria C, `gets_s()`, `scanf_s()` e `printf_s()` costituiscono valide alternative, ma restano non sicure.
...
* Le ciclicità sono difficili da capire e possono introdurre indeterminismo (comportamento non specificato).
...
##### Esempio, cattivo
    ...
    // ora si ha un'istanza di un oggetto picture non valido.
...
* sono decisamente contro-producenti
...
Ad esempio, non può avvenire nessuna allocazione o de-allocazione della memoria dopo io decollo dell'aereo (non è consentite nessun [overflow] della memoria né una frammentazione).
...
Il codice conforme non sarà la causa principale di errori per quelle proprietà, sebbene questi errori possono essere introdotti in un programma da altro codice, librerie e dall'ambiente esterno.
Un profilo può anche aggiungere ulteriori tipi di librerie per essere facilmente conformi e incoraggiare il codice corretto.
...
È possibile generare un'eccezione per indicare errori che non possono essere rilevati staticamente (al momento della compilazione).
...
Per esempio, un puntatore può essere non inizializzato, il `nullptr`, puntare oltre i limiti di un array o a un oggetto cancellato.
...
La libreria di supporto alle "Core Guidelines" è definita nel namespace `gsl` e i nomi possono essere alias per la libreria standard e di altri nomi ben noti di librerie. L'utilizzo (in fase di compilazione) del riferimento indiretto [indirection] tramite il namespace  `gsl` consente la sperimentazione e varianti locali delle funzionalità di supporto.
...
* `T*`      // Il `T*` non è proprietario, può essere nullo; si presume che punti ad un singolo elemento.
...
* `owner<T*>`   // un `T*` che possiede l'oggetto puntato/riferito; può essere `nullptr`.
...
* `not_null<T>`   // `T` è solitamente il tipo del puntatore (p.es., `not_null<int*>` o `not_null<owner<Foo*>>`) che non può essere `nullptr`.
...
Si potrebbe scrivere un ottimo danese, ma molti programmatori non lo capirebbero; i manutentori del codice potrebbero non essere danesi.
...
I nomi con i tipi codificati sono prolissi e criptici.
    ...
    printS  // scrive una std::string
    prints  // scrive una stringa C-style
    printi  // scrive un int
...
Usare suffissi letterali dove è necessario chiarire
    ...
    auto interval = 100ms;  // usando <chrono>
...
Si poteva giustificare questo abominio in C quando i prototipi delle funzioni erano rari quindi vietare questo:
    ...
    int f();
    f(1, 2, "strano ma valido nel C89");   // da sperare che f() sia definita int f(a, b, c) char* c; { /* ... */ }
...
Questi standard di codifica sono scritti usando [CommonMark](http://commonmark.org) e `<a>` HTML anchors.
...
* *algoritmo*: una procedura o una formula per rislvere un problema; una serie finita di passaggi computazionali per produrre un risultato.
...
* *classe*: un tipo definito dall'utente [user-defined] contenente dati membro, funzioni membro e tipi membro.
